...
public class DateGUI {
    ...
    private ArrayList<Date212> inputArray = new ArrayList<Date212>();
    ...
    public ArrayList<Date212> readNumbers(String fileName) throws FileNotFoundException {
        ...
        while (dateParser.hasNext()) {
            ...
            if(!(line.isEmpty())){ //ignore empty lines
                for(String day:line.split(",")){ // add each date that is on the line split by the specified delimiter
                    rawDates.add(day);
                }
            }
        }
    ...
    public void selectionSort(ArrayList<Date212> inputArray) throws ParseException {
        ...
        for (int x = 0; x < inputArray.size(); x++) { //add values to the sorted TextFrame after it has been sorted.
            //System.out.println(inputArray.get(x)); // verify if output was sorted correctly.
            sorted.append(String.valueOf(inputArray.get(x)));
            sorted.append("\n");
        }
    }
}
